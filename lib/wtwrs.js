...
report = function(request, response) {
    try {
        performRequest('/wtwman/webresources/manager/report/lat/' + request.params.lat + '/long/' + request.params.long, 'GET', {}, function(data) {
...
performRequest = function(endpoint, method, data, success) {
    ...
    console.log("!!app.get performRequest: http://" + wtwhost +  ":" + wtwport + endpoint);
    var options = {
        host : wtwhost,
        port : wtwport,
        path : endpoint,
