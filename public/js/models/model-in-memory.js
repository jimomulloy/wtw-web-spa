wtw.Employee = Backbone.Model.extend({
    ...
    sync: function(method, model, options) {
        if (method === "read") {
            wtw.store.findById(parseInt(this.id), function (data) {
...
wtw.EmployeeCollection = Backbone.Collection.extend({
    ...
    sync: function(method, model, options) {
        if (method === "read") {
            wtw.store.findByName(options.data.name, function (data) {
                options.success(data);
            });
        }
    }
...
wtw.ReportsCollection = Backbone.Collection.extend({

    model: wtw.Employee,

    sync: function(method, model, options) {
        if (method === "read") {
            wtw.store.findByManager(this.parent.id, function (data) {
                options.success(data);
            });
        }
    }
...
wtw.MemoryStore = function (successCallback, errorCallback) {
